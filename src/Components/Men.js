...
import './CSS/home.css'
import { menshoes, mendress } from './../redux/Action/actioncreator'
...
function Men(props) {
    ...
    // Api call For Men's shoes
    useEffect(() => {
        axios.get(`https://glacial-hamlet-86575.herokuapp.com/menshoes`)
            .then(res => {
                setShoes(res.data)
            }).catch(err => console.log(err))
    }, [])
...
const mapStateToProps = state => {
    return {
        shoes: state.menshoes,
        dress: state.mendress
    }
}
...
const mapdispatchToProps = dispatch => {
    return {
        addshoes: (d) => dispatch(menshoes(d)),
        adddress: (d) => dispatch(mendress(d))
    }
}
